...
MAXSCALE_IMAGE=${MAXSCALE_IMAGE:-"registry.scontain.com/sconecuratedimages/apps:maxscale-2.5-alpine"}
SOS_OSV_SCAN_IMAGE=${SOS_OSV_SCAN_IMAGE:-"registry-1.docker.io/youssefhenna/sos-osvscan:1.0.0"}
SOS_DB_MANAGER_IMAGE=${SOS_DB_MANAGER_IMAGE:-"registry-1.docker.io/youssefhenna/sos-dbmanager:1.0.0"}

SCONE_HEAP_MYSQLD=${SCONE_HEAP_MYSQLD:-"4G"}
SCONE_HEAP_MYSQL=${SCONE_HEAP_MYSQL:-"4G"}
SCONE_HEAP_PROBE=${SCONE_HEAP_PROBE:-"32M"}
...
MRENCLAVE_MYSQLD=$(get_mrenclave $MARIADB_IMAGE mysqld "-e SCONE_HEAP=$SCONE_HEAP_MYSQLD -e SCONE_ALLOW_DLOPEN=2 --entrypoint=""")
MRENCLAVE_MYSQL=$(get_mrenclave $MARIADB_IMAGE mysql "-e SCONE_HEAP=$SCONE_HEAP_MYSQL -e SCONE_ALLOW_DLOPEN=2 --entrypoint=""")
MRENCLAVE_PROBE=$(get_mrenclave $MARIADB_IMAGE mysqladmin "-e SCONE_HEAP=$SCONE_HEAP_PROBE -e SCONE_ALLOW_DLOPEN=2 --entrypoint=""")
MRENCLAVE_MAXSCALE=$(get_mrenclave $MAXSCALE_IMAGE maxscale "-e SCONE_HASH=1 -e SCONE_HEAP=$SCONE_HEAP_MAXSCALE -e SCONE_ALLOW_DLOPEN=2 --entrypoint=""")
MRENCLAVE_SOS_OSV_SCAN=$(get_mrenclave $SOS_OSV_SCAN_IMAGE /bin/osvscan "-e SCONE_HASH=1 -e SCONE_HEAP=$SCONE_HEAP_SOS_OSV_SCAN -e SCONE_ALLOW_DLOPEN=2")
MRENCLAVE_SOS_DB_MANAGER=$(get_mrenclave $SOS_DB_MANAGER_IMAGE /bin/osvdbmanager "-e SCONE_HASH=1 -e SCONE_HEAP=$SCONE_HEAP_SOS_DB_MANAGER -e SCONE_ALLOW_DLOPEN=2")
