...
public class MariadbReplicaImageValidatingPolicyDependentResource extends AbstractImageValidatingPolicyDependentResource {

    @Override
    protected CommonDependantSpec getDependantSpec(SconeOsvScanner primary) {
        return primary.getSpec().getDatabase().getMariadb();
    }

    @Override
    protected String getPolicyName(String primaryName) {
        return Constants.getMariadbReplicaImageValidatingPolicyName(primaryName);
    }
    ...
    protected CommonRegistrySpec resolveRegistrySpec(SconeOsvScanner primary) {
        ...
        return databaseSpec.getMariadb().resolveRegistrySpec(databaseSpec,primarySpec);
    }
